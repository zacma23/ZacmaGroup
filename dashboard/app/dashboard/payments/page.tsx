export default async function PaymentsPage(){
  ...
  try{
    ...
    const res = await fetch(`${apiBase}/api/v1/payments/invoices`, { cache: "no-store", headers });
            ...
            <tbody>
              {rows.map((r: any) => (
                <tr key={r.id} className="border-b border-gray-800">
                  ...
                  <td className="py-2">{r.customer_name ?? r.customer ?? "—"}</td>
                  <td className="py-2">{r.amount ?? r.total ?? "—"}</td>
                </tr>
              ))}
            </tbody>
          </table>
