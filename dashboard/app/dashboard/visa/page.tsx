...
export default async function VisaPage(){
  ...
  try{
    ...
    const res = await fetch(`${apiBase}/api/v1/visa/applications`, { cache: "no-store", headers });
    ...
    return (
      <div className="container">
        <div className="flex items-center justify-between">
          <h1 className="text-xl font-semibold">Visa — Applications</h1>
        ...
        <div className="mt-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-1 bg-slate-800 border border-gray-700 rounded p-4">
            <h2 className="text-sm font-medium text-gray-200">Recent applications</h2>
            {rows.length === 0 ? (
              <p className="mt-2 text-xs text-gray-400">No applications found for this tenant. Sample data will appear when connected.</p>
            ) : (
              <ul className="mt-3 space-y-2 max-h-72 overflow-auto">
                {rows.slice(0, 20).map((r: any) => (
                  <li key={r.id} className="text-sm text-gray-100 border-b border-gray-700 py-2">
                    <div className="font-medium">{r.applicant_name ?? r.name ?? "—"}</div>
          ...
          <div className="lg:col-span-2">
            <h3 className="text-sm text-gray-300 mb-2">Create application</h3>
            {/* @ts-ignore */}
            <ProfessionalForm
              endpoint="/api/v1/visa/applications"
              fields={[
                { name: "applicant_name", label: "Applicant name", required: true, placeholder: "Full name" },
                { name: "passport_number", label: "Passport number", placeholder: "123456789" },
                { name: "nationality", label: "Nationality", placeholder: "Country" },
    ...
    return (
      <div className="container">
        <h1 className="text-xl font-semibold">Visa</h1>
